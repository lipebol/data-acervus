...
class System:
    ...
    def notify(self, message: str) -> str or bool:
        try:
            if "@" in message:
                self._icon = self._dir + getenv('ERROR_ICON')
            return self.__shell(
    ...
    def set_wallpaper(self, args: dict) -> None:
        try:
            for key, value in args.items():
                self.__shell(getenv('SET_WALLPAPER') % (getenv(key), self._dir + getenv(value)))
        except Exception as error:
    ...
    def set_glava(self, env_dir: str) -> None:
        try:
            for file in glob(self._dir + getenv(env_dir)):
                copyfile(file, getenv('SET_GLAVA') % file.split(getenv('SLASH'))[-1])
        except Exception as error:
            self.notify(f"{error} in: @{currentframe().f_code.co_name}")


    def __shell(self, command: str) -> str or bool:
        try:
